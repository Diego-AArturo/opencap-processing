...
def cross_corr_improved(y1, y2, multCorrGaussianStd=None, window_size=None, use_fft=True, visualize=False):
    ...
    """
    Calcula la correlación cruzada y los retardos entre dos señales con mejoras adicionales.
    
    Args:
    y1, y2: Las señales de entrada, deben tener la misma longitud o se interpolarán.
    multCorrGaussianStd: Desviación estándar de la función gaussiana para ajustar la correlación cruzada.
    window_size: Tamaño de la ventana de correlación cruzada (opcional).
    use_fft: Si True, utiliza FFT para calcular la correlación cruzada.
    visualize: Si True, visualiza la correlación cruzada.
    
    Returns:
    max_corr: Correlación máxima sin normalización.
    lag: Retardo en términos del índice.
    """

    # Paso 1: Interpolación de las señales para igualar su longitud
    if len(y1) != len(y2):
        n = max(len(y1), len(y2))
        y1 = resample(y1, n)
        y2 = resample(y2, n)

    # Paso 2: Normalización de las señales
    y1 = (y1 - np.mean(y1)) / np.std(y1)
    y2 = (y2 - np.mean(y2)) / np.std(y2)

    # Paso 3: Calcular la correlación cruzada con FFT o método tradicional
    if use_fft:
        # Correlación cruzada utilizando la FFT
        corr = ifft(fft(y1) * np.conj(fft(y2))).real
    else:
        # Correlación cruzada utilizando el método tradicional
        corr = correlate(y1, y2, mode='same')
    
    # Paso 4: Aplicar una ventana si está definida
    if window_size is not None:
        # Asegurar que la longitud de la ventana sea igual a la de la correlación
        if window_size > len(corr):
            window_size = len(corr)

        window = np.hanning(window_size)
        
        # Ajustar la longitud de la ventana al tamaño de 'corr'
    ...
    # Paso 5: Multiplicar por una gaussiana si se especifica
    if multCorrGaussianStd is not None:
        gauss_window = gaussian(len(corr), multCorrGaussianStd)
        corr = corr * gauss_window

    # Paso 6: Calcular el retardo y la correlación máxima
    shift = len(y1) // 2
    max_corr = np.max(corr)
    argmax_corr = np.argmax(corr)
    lag = argmax_corr - shift
    ...
    if visualize:
        plt.figure()
        plt.plot(corr)
        plt.title('Correlación cruzada mejorada')
        plt.xlabel('Retardo')
        plt.ylabel('Correlación')
        plt.axvline(x=argmax_corr, color='r', linestyle='--', label='Retardo máximo')
        plt.legend()
        plt.show()
