...
def IntegrateForcepalte_v1(session_id, trial_name, force_gdrive_url,participant_id):
...
# to right and left foot.
    ...
    center_of_mass_pos = kinematics.get_body_angular_velocity(body_names=['pelvis'], lowpass_cutoff_frequency=4,expressed_in='ground') # other option
    # center_of_mass_pos = kinematics.get_center_of_mass_speeds(lowpass_cutoff_frequency=4)
    # print(kinematics.get_coordinate_values(lowpass_cutoff_frequency=4).columns)
    ...
    #print(center_of_mass_acc[center_of_mass_acc['time']>= 0.066667])
    ...
    dif_lengths = len(forces_for_cross_corr_downsamp) - len(center_of_mass_pos['pelvis_y'])
    
    if dif_lengths > 0:
        
        com_signal = np.pad(center_of_mass_pos['pelvis_y'] , 
        ...
        com_signal = center_of_mass_pos['pelvis_y'].values[:, np.newaxis] 
        ...
        com_signal = center_of_mass_pos['pelvis_y'][:, np.newaxis] 
        force_signal = forces_for_cross_corr_downsamp
        kinematics_pad_length = 0
    ...
    _,lag = ut.cross_corr(np.squeeze(com_signal),np.squeeze(force_signal),  #linea original donde el max_corr era _ y es force_signal
    ...
    force_data_new[:,0] = force_data[:,0] - (lag / framerate_kinematics)
    ...
    if save_plot:
        ...
        save_path = os.path.join(save_folder, f"{trial_name}_angular_velocity.png")

        # Crear la figura y graficar los datos
        plt.figure()
        plt.plot(kinematics.time, com_signal , label='COM')
        plt.plot(force_data_new[:, 0], forces_for_cross_corr, label='vGRF')
        plt.xlabel('Time (s)')
        plt.legend()
        plt.grid()
